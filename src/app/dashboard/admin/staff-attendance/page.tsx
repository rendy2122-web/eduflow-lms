...
export default function AdminStaffAttendancePage() {
  ...
  // 3. User session info & dynamic simulation states
  const currentUserLog = logs.find(log => log.isMock === false && log.role === 'Sistem Admin');
  ...
  const handleStatusOverride = async (staffId: string, status: string) => {
    try {
      const res = await fetch('/api/admin/staff-attendance', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        showSuccess(`Status kehadiran berhasil diubah menjadi ${statusLabel}! 📋`);
        await fetchData();
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <DashboardLayout activeMenu="staff-attendance" pageTitle="Absensi Guru & Staf" pageSubtitle={`Pemantauan Kehadiran Staf — ${formatDateDisplay(selectedDate)}`}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px', animation: 'fadeInUp 0.4s ease-out' }}>
        ...
        {/* ════════════════════════════════════════════════════════════════
           ...
           ════════════════════════════════════════════════════════════════ */}
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', flexWrap: 'wrap' }}>
          {/* Card 1: Total Staf */}
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
          ...
          <div className="card-premium" style={{ 
            ...
            <span style={{ fontSize: '0.68rem', fontWeight: 800, backgroundColor: 'rgba(99, 102, 241, 0.2)', color: '#818cf8', padding: '4px 12px', borderRadius: '99px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Simulasi Absensi Staf</span>
        ...
        {/* ════════════════════════════════════════════════════════════════
             SECTION 4: INTERACTIVE LOG TABLE
           ════════════════════════════════════════════════════════════════ */}
        <section className="card-premium" style={{ padding: 0, overflow: 'hidden', border: '1px solid #e2e8f0', borderRadius: '20px' }}>
          ...
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ fontSize: '0.92rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>Log Kehadiran Staf Hari Ini</h2>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #e2e8f0', backgroundColor: '#f8fafc', color: '#64748b', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    ...
                    <th style={{ padding: '14px 24px', textAlign: 'right' }}>Aksi Admin</th>
                  </tr>
                </thead>
                <tbody>
                  {logs.map((log, idx) => {
                        ...
                        <td style={{ padding: '12px 24px', textAlign: 'right' }}>
                          <select
                            ...
                            style={{
                              ...
                              fontSize: '0.75rem', fontWeight: 600, color: '#475569', backgroundColor: '#ffffff',
