...
export default function OrangTuaRaporPage() {
  ...
  const fetchChildrenList = useCallback(async () => {
    try {
      ...
      if (json.success) {
        const childList = Object.values(json.children || {}).map((c: any) => ({
          id: c.id,
          name: c.name,
          class: c.class,
          nisn: c.nisn
      ...
      console.error('Error fetching child list for parent report card:', err);
    } finally {
  ...
  // Fetch report card data for selected child
  const fetchRapor = useCallback(async () => {
    if (!selectedChildId) return;

    try {
      setLoadingRapor(true);
      const res = await fetch(`/api/guru/rapor?studentId=${selectedChildId}`);
      const json = await res.json();
      if (json.success) {
        setRaporData(json);
      } else {
      ...
      console.error('Error fetching child report card details:', err);
    } finally {
  ...
  return (
    <DashboardLayout activeMenu="reports" pageTitle="Rapor Hasil Belajar Anak" pageSubtitle="Unduh lembar penilaian resmi dan ikuti progres akademik anak Anda harian.">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }} className="no-print">
        ...
        <section className="card-premium" style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '18px', padding: '20px', display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center' }}>
          <div className="form-group" style={{ margin: 0, minWidth: '220px' }}>
            <label className="form-label" style={{ fontSize: '0.75rem', fontWeight: 700, color: '#64748b', marginBottom: '6px' }}>Pilih Nama Anak</label>
            <select
              className="form-control"
              value={selectedChildId}
              onChange={(e) => setSelectedChildId(e.target.value)}
              disabled={children.length === 0}
              style={{ width: '220px', height: '42px', padding: '0 12px', borderRadius: '8px', border: '1px solid #cbd5e1' }}
            >
              {children.length === 0 ? (
        ...
        {loadingRapor ? (
          ...
          <div style={{ padding: '64px', textAlign: 'center', color: '#64748b' }}>
            Data Rapor anak Anda tidak ditemukan.
          </div>
        )}
