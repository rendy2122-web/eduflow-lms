...
export default function OrangTuaTahfidzPage() {
  ...
  const fetchChildren = useCallback(async () => {
    try {
      ...
      console.error('Error fetching parent dashboard children info:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // Fetch Tahfidz records for selected child
  const fetchTahfidzRecords = useCallback(async (studentId: string) => {
    try {
      ...
      console.error('Error fetching child tahfidz records:', error);
    } finally {
  ...
  useEffect(() => {
    if (child && child.id) {
      fetchTahfidzRecords(child.id);
    } else {
      setRecords([]);
    }
  }, [selectedChild, children, child, fetchTahfidzRecords]);
    ...
    <DashboardLayout activeMenu="tahfidz">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-6)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--spacing-4)' }}>
          <div>
            <h1 style={{ font: 'var(--font-h2)' }}>Jurnal Perkembangan Quran Tahfidz Anak</h1>
            <p style={{ color: 'hsl(var(--color-neutral-gray))', fontSize: '0.9rem' }}>
              Pantau perjalanan setoran hafalan, review sabqi/manzil, dan koreksi tajwid langsung dari ustadz pembimbing.
            </p>
          </div>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'var(--spacing-4)' }}>
              ...
              <div className="card-premium" style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-4)' }}>
                ...
                <div>
                  <p style={{ color: 'hsl(var(--color-neutral-gray))', fontSize: '0.85rem' }}>Total Setoran Anak</p>
              ...
              <div className="card-premium" style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-4)' }}>
                ...
                <div>
                  <p style={{ color: 'hsl(var(--color-neutral-gray))', fontSize: '0.85rem' }}>Kelancaran Rata-rata</p>
            ...
            {!recordsLoading && totalSetoran > 0 && (
              <div className="card-premium" style={{ 
                ...
                <div>
                  <h4 style={{ font: 'var(--font-h4)', margin: '0 0 4px 0', color: 'hsl(var(--color-primary-dark))' }}>Ringkasan Perjalanan Hafalan Anak</h4>
                  <p style={{ margin: 0, fontSize: '0.9rem', lineHeight: '1.6', color: '#374151' }}>
                    Alhamdulillah, <strong>{child.name}</strong> telah menyelesaikan <strong>{totalSetoran} kali</strong> setoran hafalan Al-Qur&apos;an. 
                    Setoran hafalan baru (Ziyadah) terakhirnya adalah <strong>{latestZiyadahStr}</strong>. 
                    Tingkat kelancaran rata-rata hafalan anak Anda mencapai <strong>{averageKelancaran}%</strong> yang masuk dalam kategori <strong>{getPredicate(averageKelancaran)}</strong>. 
                    {lastMurajaah && ` Kegiatan murajaah terakhir dilakukan pada tanggal ${new Date(lastMurajaah.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}.`}
                    {' '}Bantu dukung anak Anda untuk berlatih di rumah berdasarkan catatan evaluasi guru penyimak di bawah.
                  </p>
                </div>
              </div>
            )}
              ...
              <div style={{ overflowX: 'auto' }}>
                {recordsLoading ? (
                  <div style={{ padding: '40px', textAlign: 'center', color: 'hsl(var(--color-neutral-gray))' }}>
                    Mengambil data riwayat setoran...
                  </div>
                ) : filteredRecords.length === 0 ? (
                  <div style={{ padding: '40px', textAlign: 'center', color: 'hsl(var(--color-neutral-gray))' }}>
                    Tidak ada data riwayat setoran pada kategori ini untuk {child.name}.
                  </div>
                ) : (
