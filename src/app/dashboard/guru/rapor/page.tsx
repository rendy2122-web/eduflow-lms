...
export default function GuruRaporPage() {
  ...
  // Fetch reports for statistics overview
  ...
  return (
    <DashboardLayout activeMenu="rapor" pageTitle="Kelola Rapor Digital" pageSubtitle="Pilih kelas dan siswa untuk melihat, mengedit, dan mencetak rapor lengkap (nilai akademik, kehadiran, tahfidz, dan catatan wali kelas).">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }} className="no-print">
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
            <div>
              ...
              <p style={{ fontSize: '3rem', fontWeight: 800, margin: 0, lineHeight: 1 }}>{classAverage}</p>
            ...
            <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: '0.72rem', opacity: 0.9, margin: '0 0 4px 0' }}>Tertinggi</p>
                <p style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, color: '#10b981' }}>{highestScore}</p>
              </div>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontSize: '0.72rem', opacity: 0.9, margin: '0 0 4px 0' }}>Terendah</p>
                <p style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, color: '#ef4444' }}>{lowestScore}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              {students.length === 0 ? (
                ...
                students.map((student) => (
                  <option key={student.id} value={student.id}>{student.nama} (NISN. {student.nisn})</option>
                ))
              )}
            </select>
          </div>
        ...
        {/* Student List with Expandable Cards */}
        {reports.length > 0 && (
          <div style={{
            background: '#ffffff',
            ...
            <div style={{ padding: '20px 24px', background: 'linear-gradient(135deg, #f8fafc, #f1f5f9)', borderBottom: '1px solid #e2e8f0' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>
                Daftar Siswa ({reports.length})
              </h3>
              <p style={{ fontSize: '0.8rem', color: '#64748b', margin: '4px 0 0 0' }}>
                Klik pada kartu siswa untuk melihat detail nilai
              </p>
            </div>
                ...
                >
                  <div
                    style={{
                      ...
                      alignItems: 'center',
                      background: expandedStudent === report.student.id ? '#f8fafc' : '#ffffff',
                      transition: 'background 0.2s ease'
                    }}
                    onClick={() => {
                      setExpandedStudent(expandedStudent === report.student.id ? null : report.student.id);
                      setSelectedStudentId(report.student.id);
                    }}
                    onMouseEnter={(e) => { if (expandedStudent !== report.student.id) e.currentTarget.style.background = '#f8fafc'; }}
                    onMouseLeave={(e) => { if (expandedStudent !== report.student.id) e.currentTarget.style.background = '#ffffff'; }}
                  >
                              ...
                              >
                                <div style={{ flex: 1 }}>
                                  ...
                                  <p style={{ fontSize: '0.72rem', color: '#64748b', margin: 0 }}>
                                    {grade.type} • {new Date(grade.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                                  </p>
                                </div>
                                <div style={{
                                  display: 'flex',
